...
export const spread = (...args: unknown[]) => {
  ...
  const normalizedChildren = unwrapLodashArray(
    (children ?? []) as GoFishAST[] | Collection<GoFishAST>
  );
  ...
  return createGoFishPrimitive(
    {
      ...
      resolveUnderlyingSpace: (
        ...
        if (alignSpaces.every((s) => isSIZE(s))) {
          alignFromSize = true;
          const sizeValues = alignSpaces.map((s) => (s as any).value as number);
        ...
        // if children are all SIZE and spacing is 0, return POSITION (sum of sizes)
          ...
          const sizeValues = stackSpaces.map((s) => (s as any).value as number);
      ...
      layout: (
        ...
        if (!posScales?.[alignDir] || alignFromSize || alignment === "middle") {
          if (alignment === "start") {
            const baseline =
